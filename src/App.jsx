...
// BASIC 1.0.9 로그인 안정화판: 구매자 이메일 인증 설정
// Vercel 환경변수에 아래 2개를 등록해야 합니다.
...
const AUTH_DEBUG_INFO={
  url:SUPABASE_URL||'(비어 있음)',
  keyType:SUPABASE_ANON_KEY?(/^eyJ/.test(SUPABASE_ANON_KEY)?'Legacy anon public key':'Publishable key / 기타 키'):'(비어 있음)',
  keyPrefix:SUPABASE_ANON_KEY?`${SUPABASE_ANON_KEY.slice(0,12)}…`:'(비어 있음)'
};
function readableSupabaseError(error){
  ...
  if(/Failed to fetch/i.test(raw))return 'Supabase 주소가 잘못되었거나 네트워크 연결이 차단되었습니다. VITE_SUPABASE_URL 값을 다시 확인해 주세요.';
  if(/Invalid API key|apikey|JWT|Unauthorized/i.test(raw))return 'Supabase 공개키가 맞지 않습니다. VITE_SUPABASE_ANON_KEY에 Legacy anon public key를 넣었는지 확인해 주세요.';
  return raw;
}
...
async function supabaseFetch(path,{method='GET',token,body,query}={}){
  if(!SUPABASE_READY)throw new Error('Supabase 환경변수가 설정되지 않았습니다.');
  ...
  if(!res.ok){
    ...
    const err=new Error(detail||`Supabase 요청 실패: ${res.status}`);
...
async function requestMagicLink(email){
  ...
  if(!SUPABASE_URL||!/https:\/\/.+\.supabase\.co$/.test(SUPABASE_URL)){
    throw new Error(`VITE_SUPABASE_URL 값이 올바르지 않습니다. 현재 값: ${SUPABASE_URL||'(비어 있음)'}`);
  }
  if(!SUPABASE_ANON_KEY){
    throw new Error('VITE_SUPABASE_ANON_KEY 값이 비어 있습니다.');
  }
  // Supabase Auth GoTrue REST API: magic link / OTP 발송
  return supabaseFetch('/auth/v1/otp',{method:'POST',query:{redirect_to:authRedirectUrl()},body:{email:clean,create_user:true}});
}
async function getAuthUser(session){
  return supabaseFetch('/auth/v1/user',{token:session?.access_token});
}
async function checkAllowedBuyer(email,session){
  const clean=normalizeEmail(email);
  const data=await supabaseFetch('/rest/v1/allowed_users',{
    token:session?.access_token,
    query:{select:'email,plan,active,church_name,memo',email:`eq.${clean}`,active:'eq.true',limit:'1'}
  });
...
function AuthGate({children}){
  ...
  useEffect(()=>{
    let cancelled=false;
    async function boot(){
      if(!SUPABASE_READY){setStatus('setup');return;}
      setStatus('checking');
      setError('');
      try{
...
const BASIC_DOC_TYPES=['기본 공지 안내문','각부 월간행사 안내','교육부서 주간보고서'];
const CATEGORIES=[
...
const DEFAULT_LABELS={
  ...
  '교육부 주간보고서':['교육부 주간 활동 요약','부서별 주간 현황','공동 기도제목','공유/지원 요청'],
  '교육부서 주간보고서':['예배/모임 현황','이번 주 활동','다음 주 계획','특이사항 및 지원 요청','기도제목'],
...
function budgetRow(){return {item:'',detail:'',qty:'',price:'',note:''}}
function budgetTotal(rows){return (rows||[]).reduce((s,r)=>s+num(r.qty)*num(r.price),0)}
...
부모와 교회가 함께 다음세대를 세우도록`};
  ...
  if(type==='교육부 주간보고서'){const d={reportTitle:'교육부 주간보고서',period:'2026년 6월 3주',writer:'교육부 담당 교역자 김선규 목사',summary:'이번 주 교육부는 각 부서 예배와 공과를 정상적으로 진행했습니다. 여름사역 준비를 위해 부서별 일정, 교사 배치, 예산 사용 계획을 1차 점검했으며, 다음 주에는 전체교사 기도회와 부서별 준비모임을 통해 세부 역할을 확정할 예정입니다.',commonPrayer:'다음세대가 예배와 말씀 안에서 하나님을 인격적으로 만나도록\n교사들이 지치지 않고 기쁨과 사랑으로 아이들을 섬기도록\n여름성경학교와 수련회 준비 과정에 안전과 지혜를 더하시도록',support:'부서별 여름행사 일정과 예산안을 이번 주 안에 최종 확인해 주세요.\n전체교사 기도회 참석을 각 부서에서 한 번 더 독려해 주세요.\n미정인 봉사자와 차량 지원 여부를 교육부 담당자에게 공유해 주세요.'};const samples={영아부:['12','부모 동반 예배 및 말씀놀이 진행','부모 안내문 발송, 여름성경학교 물품 확인','새가정 1가정 방문, 적응 지원 필요'],유치부:['24','공과 활동 및 찬양 율동 연습','여름성경학교 찬양곡 선정, 반별 담당 확인','보조교사 1명 추가 지원 요청'],초등부:['38','말씀암송 활동과 조별 나눔 진행','여름성경학교 조 편성, 준비물 목록 정리','활동 재료 구입 전 예산 확인 필요'],청소년부:['31','소그룹 나눔 및 수련회 주제 안내','수련회 신청 독려, 찬양팀 연습 시작','신입생 출석 관리와 연락 필요'],청년부:['28','청년부 예배 후 소그룹 모임 진행','수련회 역할분담표 작성, 기도회 준비','새가족 2명 정착을 위한 식사교제 예정']};EDU_DEPTS.forEach((x)=>{const r=samples[x]||['','이번 주 활동 입력','다음 주 계획 입력','특이사항 입력'];d[`${x}_attendance`]=r[0];d[`${x}_this`]=r[1];d[`${x}_next`]=r[2];d[`${x}_special`]=r[3]});return d;}
  if(type==='교육부서 주간보고서') return {reportTitle:'교육부서 주간보고서',period:'2026년 7월 1주',subDepartment:'청소년부',writer:'담당 교역자',attendance:'24',worship:'주일예배 및 분반공부 진행',thisWeek:`주일예배와 분반공부를 진행했습니다.
여름수련회 참석 신청을 안내했습니다.
...
여름수련회 준비가 안전하고 은혜롭게 진행되도록`};
  ...
  if(type==='공문/협조 요청서') return {title:'교육부 여름사역 협조 요청서',docNo:'교육부-2026-06',date:'2026년 6월 20일',sender:'교육부 담당 교역자',receiver:'각 부서장 및 행정 담당자',subject:'2026년 교육부 여름사역 준비 협조 요청',background:'2026년 교육부 여름사역을 안전하고 은혜롭게 준비하기 위해 부서별 일정, 공간 사용, 차량, 예산 집행 관련 사항을 사전에 조율하고자 합니다.',request:'부서별 여름행사 일정과 장소 사용 계획을 공유해 주시기 바랍니다.\n차량 운행, 방송 장비, 식사 준비 등 협조가 필요한 항목을 미리 알려주시기 바랍니다.\n예산 집행 예정 항목은 재정부 확인 후 진행해 주시기 바랍니다.',schedule:'제출 기한: 2026년 6월 28일(주일)\n확인 회의: 2026년 6월 30일(화) 오후 7시 30분\n최종 공유: 2026년 7월 5일(주일)',reply:'회신 방법: 교육부 담당 교역자에게 카카오톡 또는 이메일로 전달\n문의: 교육부 담당 교역자 김선규 목사',closing:'교회학교와 다음세대 사역을 위해 함께 협력해 주셔서 감사합니다.'};
...
function merge(saved){const b=makeAll();if(!saved||typeof saved!=='object')return b;const legacy={...saved};if(legacy['7개부서 주간보고서']&&!legacy['7개부서 보고서']) legacy['7개부서 보고서']=legacy['7개부서 주간보고서'];
...
function titlePathOf(type){
  ...
  if(['교육부 주간보고서','교육부서 주간보고서','7개부서 보고서'].includes(type)) return 'reportTitle';
...
function metaPathOf(type){
  ...
  if(['주간 공지','교육부 주간보고서','교육부서 주간보고서','7개부서 보고서','기획위원회 보고서','예산안','일정표','세부 프로그램 문서','행사 결과 보고서'].includes(type)) return 'period';
...
function AttendanceNumber({path,value}){const n=childAttendanceValue(value);return <div className="attendance-number-cell" {...fontTargetAttrs(`attendance:${path}`,'아이들 출석 숫자')}><Edit path={path} value={n} className="attendance-number-edit"/><span className="attendance-unit">명</span></div>}
...
const EDUCATION_DEPT_WEEKLY_SCHEMA=[
  ...
  {field:'attendance',label:'아이들 출석',aliases:[/아이들\s*출석|아이들출석|아동\s*출석|학생\s*출석|출석\s*인원|출석/],multiline:false},
...
function parseEducationDeptWeekly(doc,lines,title){
  ...
  const next={...doc};
  next.reportTitle=fields.reportTitle||reportLine||doc.reportTitle||title||'교육부서 주간보고서';
...
function parseEducationWeekly(doc,lines,title){
  ...
  EDU_DEPTS.forEach(dep=>{
    ...
    depLines.forEach(line=>{
      ...
      const att=matchFieldLine(stripped,[/아이들\s*출석|아이들출석|출석\s*인원|출석/]);
...
function aliasesForQuickPath(path,label,type){
  ...
  if(p.includes('attendance'))return add(['아이들 출석','아이들출석','출석','출석 인원','출석인원','아동 출석','학생 출석']);
...
function applyGeneralKeyedSmartFields(type,doc,text){
  ...
  if(type==='교육부서 주간보고서'&&!fields.subDepartment){
...
function smartPasteExampleFor(type){
  ...
  if(type==='교육부서 주간보고서')return '영아부 주간보고서\n아이들출석 8\n기간 2026.07.14\n작성자 김선규\n이번주 활동 = 분반\n다음주 계획 - 야외예배\n특이사항 - 없음\n기도제목 - 다음세대를 위해';
  if(type==='교육부 주간보고서')return '교육부 주간보고서\n기간 2026.07.14\n작성자 김선규\n영아부 아이들출석 8\n영아부 이번주 활동 분반\n영아부 다음주 계획 야외예배\n유치부 아이들출석 12\n공동 기도제목 - 다음세대를 위해';
  return '제목 또는 행사명\n주요 내용 1\n주요 내용 2\n요청사항 또는 기도제목';
}
function summarizeSmartPasteDoc(type,doc){
  ...
  if(type==='교육부서 주간보고서')return [`제목: ${doc.reportTitle||'-'}`,`부서: ${doc.subDepartment||'-'}`,`아이들 출석: ${doc.attendance||'-'}명`,`기간: ${doc.period||'-'}`,`작성자: ${doc.writer||'-'}`,`이번 주 활동: ${doc.thisWeek||'-'}`,`다음 주 계획: ${doc.nextWeek||'-'}`,`특이사항: ${doc.special||'-'}`,`기도제목: ${doc.prayer||'-'}`];
  if(type==='교육부 주간보고서')return [`제목: ${doc.reportTitle||'-'}`,`기간: ${doc.period||'-'}`,`작성자: ${doc.writer||'-'}`,`영아부 출석: ${doc['영아부_attendance']||'-'}명`,`공동 기도제목: ${splitNonEmpty(doc.commonPrayer).length}줄`];
...
function quickFieldsFor(type,doc={}){
  ...
  if(type==='교육부서 주간보고서')return [
    quickField('문서 제목','reportTitle'),quickField('부서','subDepartment','select',EDU_DEPTS),quickField('아이들 출석','attendance'),quickField('기간','period'),quickField('작성자','writer'),quickField('이번 주 활동','thisWeek','area'),quickField('다음 주 계획','nextWeek','area'),quickField('특이사항','special','area'),quickField('기도제목','prayer','area')
  ];
  if(type==='교육부 주간보고서')return [quickField('문서 제목','reportTitle'),quickField('기간','period'),quickField('작성자','writer'),quickField('교육부 주간 활동 요약','summary','area'),quickField('공동 기도제목','commonPrayer','area'),quickField('공유/지원 요청','support','area')];
...
function v2RequiredLabels(type){
  ...
  if(type==='교육부서 주간보고서')return ['부서','아이들 출석','기간','작성자','이번 주 활동','다음 주 계획','특이사항','기도제목'];
  if(type==='교육부 주간보고서')return ['기간','작성자','교육부 주간 활동 요약','공동 기도제목','공유/지원 요청'];
...
function v25DaysForPreset(preset){
  if(preset==='당일 행사')return ['1일차'];
  if(preset==='1박 2일')return ['1일차','2일차'];
  if(preset==='2박 3일')return ['1일차','2일차','3일차'];
  if(preset==='3박 4일')return ['1일차','2일차','3일차','4일차'];
  return ['1일차','2일차','3일차'];
}
function v25PresetFromDays(doc){
  const n=(doc.days||[]).length||1;
  if(doc.schedulePreset)return doc.schedulePreset;
  if(n<=1)return '당일 행사';
  if(n===2)return '1박 2일';
  if(n===3)return '2박 3일';
  if(n>=4)return '3박 4일';
...
function V25ScheduleSetup({doc,setDoc}){
  ...
  return <section className="v25-schedule-setup">
    <div className="v25-page-card-title"><b>일정표 설정</b><span>몇 박 며칠인지와 시간표 기준을 먼저 정합니다.</span></div>
    ...
    <div className="v25-mini-grid">
      ...
      <V21Select label="일차" value={days[0]} options={days} onChange={()=>{}}/>
    </div>
  </section>
}
...
function V21ScheduleBlock({doc,setDoc}){const rows=doc.scheduleItems?.length?doc.scheduleItems:[schedRow(doc.days?.[0]||'1일차')];const days=doc.days?.length?doc.days:['1일차'];function setRows(next){setDoc({...doc,scheduleItems:next})}function patch(i,k,v){setRows(updateArray(rows,i,k,v))}function add(){const day=days[0]||'1일차';setRows([...rows,schedRow(day)])}return <section className="v21-auto-block v25-page-schedule-editor v229-schedule-editor"><V25ScheduleSetup doc={doc} setDoc={setDoc}/><div className="v21-block-head"><div><b>상세 일정표</b><span>일차·시간·일정명·장소를 먼저 넓게 입력합니다. 일정표 글씨 크기는 위 설정에서 바로 조절하세요. 담당/메모는 필요할 때만 더보기에서 수정하세요.</span></div><button type="button" onClick={add}>+ 일정 추가</button></div>{rows.map((r,i)=><V229ScheduleRowEditor key={i} row={r} index={i} days={days} total={rows.length} onDelete={()=>setRows(rows.filter((_,idx)=>idx!==i))} onPatch={(k,v)=>patch(i,k,v)}/>)}</section>}
...
function v23CoreFields(type,doc={}){
  ...
  if(type==='교육부서 주간보고서')return [v23Field('제목','reportTitle'),v23Field('부서','subDepartment','select',EDU_DEPTS),v23Field('출석','attendance'),v23Field('기간','period'),v23Field('작성자','writer')];
  if(type==='교육부 주간보고서')return [v23Field('제목','reportTitle'),v23Field('기간','period'),v23Field('작성자','writer')];
...
function docFormatRule(type){
  ...
  if(['회의자료','연말/연차 부서 보고서','행사 및 수련회 기획안','행사 결과 보고서','교육부 주간보고서','교육부서 주간보고서','7개부서 보고서','기획위원회 보고서'].includes(type))return {kind:'fixed',title:'페이지 고정형',desc:'페이지별 역할을 유지하며 작성하는 문서입니다.',modes:['페이지 역할 유지','넘치면 추가 페이지']};
...
function v23BaseTabsFor(type,doc){
  ...
  if(type==='교육부서 주간보고서')return [
    {id:'basic',label:'기본 정보',title:'주간보고 기본 정보',desc:'부서, 기간, 작성자, 출석을 수정합니다.',kind:'fields',fields:v25Fields(['제목','reportTitle'],['부서','subDepartment','select',EDU_DEPTS],['아이들 출석','attendance'],['기간','period'],['작성자','writer'])},
  ...
  if(type==='교육부 주간보고서')return [
    {id:'basic',label:'기본 정보',title:'교육부 주간보고 기본 정보',desc:'제목, 기간, 작성자를 수정합니다.',kind:'fields',fields:v25Fields(['제목','reportTitle'],['기간','period'],['작성자','writer'])},
    {id:'summary',label:'요약·기도',title:'교육부 주간 활동 요약',desc:'공동 보고 내용, 기도제목, 지원요청을 작성합니다.',kind:'fields',fields:v25Fields(['활동요약','summary','area'],['기도제목','commonPrayer','area'],['지원요청','support','area'])},
    {id:'departments',label:'부서별 현황',title:'부서별 주간 현황',desc:'영아부부터 청년부까지 출석, 이번 주, 다음 주, 특이사항을 수정합니다.',kind:'fields',fields:v25Fields(...EDU_DEPTS.flatMap(dep=>[['출석 '+dep,`${dep}_attendance`],['이번 주 '+dep,`${dep}_this`,'area'],['다음 주 '+dep,`${dep}_next`,'area'],['특이사항 '+dep,`${dep}_special`,'area']]))}
  ];
...
function V237FormAdjustPanel({type,doc,setDoc}){
  const hasAdjust=['기본 공지 안내문','주간 공지','각부 월간행사 안내','교육부서 주간보고서','교육부 주간보고서','7개부서 보고서','행사 및 수련회 기획안','세부 프로그램 문서','예산안','준비목록',CUE_DOC,'회의자료'].includes(type);
...
function MobileDocPicker({type,setType,setSelected,setStage}){
  ...
  const favorites=['기본 공지 안내문','각부 월간행사 안내','교육부서 주간보고서'];
...
function FirstUsePanel({type,setType,setSelected,easyMode,setEasyMode,onPDF,onPNG,busy,savedAt}){
  const cards=[
    ...
    {label:'교육부서 주간보고서',doc:'교육부서 주간보고서',desc:'출석·주간 활동·다음 계획·기도제목을 보고서로 정리'}
  ];
...
function mobileQuickMeta(type,tab){
  ...
  if(type==='교육부 주간보고서'){
    ...
    if(tab?.id==='summary')return {title:'보고요약',desc:'교육부 주간 활동 요약'};
...
function BudgetRowsTable({rows,onChange,addLabel='항목 추가'}){
  ...
  return <>
    <div className="budget-scroll-guide"><span>좌우로 밀어서 항목 전체를 수정할 수 있습니다.</span><em>항목 · 내용 · 수량 · 단가 · 비고</em></div>
    <div className="budget-table-scroll" tabIndex={0} aria-label="예산 입력 표 좌우 스크롤">
      <div className="table-input budget-table-input">
        <div className="thead"><b>항목</b><b>내용</b><b>수량</b><b>단가</b><b>비고</b><b></b></div>
        {safe.map((r,i)=><div className="trow" key={i}>
          <input value={r.item} onChange={e=>onChange(updateArray(safe,i,'item',e.target.value))}/>
          <input value={r.detail} onChange={e=>onChange(updateArray(safe,i,'detail',e.target.value))}/>
          <input value={r.qty} onChange={e=>onChange(updateArray(safe,i,'qty',e.target.value))}/>
          <input value={r.price} onChange={e=>onChange(updateArray(safe,i,'price',e.target.value))}/>
          <input value={r.note} onChange={e=>onChange(updateArray(safe,i,'note',e.target.value))}/>
          <button disabled={safe.length<2} onClick={()=>onChange(safe.filter((_,idx)=>idx!==i))}>삭제</button>
        </div>)}
      </div>
    </div>
    <button className="btn secondary" onClick={()=>onChange([...safe,budgetRow()])}>{addLabel}</button>
    <p className="hint">합계: <b>{won(budgetTotal(safe))}</b></p>
  </>
}
function BudgetEditor({title,rows,onChange}){return <Box title={title}><BudgetRowsTable rows={rows} onChange={onChange}/></Box>}
function BudgetPairEditor({incomeRows,expenseRows,onIncomeChange,onExpenseChange,title='수입·지출 예산 입력'}){return <Box title={title} className="budget-pair-box"><p className="hint">수입과 지출을 한 번에 열어 수정할 수 있습니다. 각 항목은 필요할 때 줄만 추가하세요.</p><div className="budget-pair-editor"><section><h4>수입 예산</h4><BudgetRowsTable rows={incomeRows} onChange={onIncomeChange} addLabel="수입 항목 추가"/></section><section><h4>지출 예산</h4><BudgetRowsTable rows={expenseRows} onChange={onExpenseChange} addLabel="지출 항목 추가"/></section></div></Box>}
...
function renderEditor(type,d,setD){
  ...
  if(type==='교육부 주간보고서') return <><Box title="기본 정보"><div className="grid3"><Field label="제목" value={d.reportTitle} onChange={v=>setD({...d,reportTitle:v})}/><Field label="기간" value={d.period} onChange={v=>setD({...d,period:v})}/><Field label="작성자" value={d.writer} onChange={v=>setD({...d,writer:v})}/></div><Area label={labelOf(d,0,'교육부 주간 활동 요약')} value={d.summary} onChange={v=>setD({...d,summary:v})}/></Box><Box title={labelOf(d,1,'부서별 주간 현황')}>{EDU_DEPTS.map(dep=><div className="dept-edit" key={dep}><h4>{dep}</h4><div className="grid4"><Field label="아이들 출석(명)" type="number" value={childAttendanceValue(d[`${dep}_attendance`])} onChange={v=>setD({...d,[`${dep}_attendance`]:v})}/><Field label="이번 주 활동" value={d[`${dep}_this`]} onChange={v=>setD({...d,[`${dep}_this`]:v})}/><Field label="다음 주 활동" value={d[`${dep}_next`]} onChange={v=>setD({...d,[`${dep}_next`]:v})}/><Field label="특이사항" value={d[`${dep}_special`]} onChange={v=>setD({...d,[`${dep}_special`]:v})}/></div></div>)}</Box><Area label={labelOf(d,2,'공동 기도제목')} value={d.commonPrayer} onChange={v=>setD({...d,commonPrayer:v})}/><Area label={labelOf(d,3,'공유/지원 요청')} value={d.support} onChange={v=>setD({...d,support:v})}/></>;
  if(type==='교육부서 주간보고서') return <><StaticBox title="기본 정보"><div className="grid4"><Field label="제목" value={d.reportTitle} onChange={v=>setD({...d,reportTitle:v})}/><Select label="부서" value={d.subDepartment} options={EDU_DEPTS} onChange={v=>setD({...d,subDepartment:v})}/><Field label="기간" value={d.period} onChange={v=>setD({...d,period:v})}/><Field label="작성자" value={d.writer} onChange={v=>setD({...d,writer:v})}/></div><Field label="아이들 출석(명)" type="number" value={childAttendanceValue(d.attendance)} onChange={v=>setD({...d,attendance:v})}/></StaticBox><Area label={labelOf(d,1,'이번 주 활동')} value={d.thisWeek} onChange={v=>setD({...d,thisWeek:v})}/><Area label={labelOf(d,2,'다음 주 계획')} value={d.nextWeek} onChange={v=>setD({...d,nextWeek:v})}/><Area label={labelOf(d,3,'특이사항 및 지원 요청')} value={d.special} onChange={v=>setD({...d,special:v})}/><Area label={labelOf(d,4,'기도제목')} value={d.prayer} onChange={v=>setD({...d,prayer:v})}/></>;
  ...
  if(type==='행사 및 수련회 기획안') return <>
    ...
    <SimpleScheduleEditor doc={d} setDoc={setD} hideNotice={true}/>
...
function MoneyTable({rows}){const safe=rows||[];return <Table heads={['항목','내용','수량','단가','합계','비고']} rows={safe.map(r=>[blank(r.item),blank(r.detail),blank(r.qty),num(r.price).toLocaleString('ko-KR'),won(num(r.qty)*num(r.price)),blank(r.note)])}/>}
...
function WeeklyPreview({doc,type}){if(type==='교육부 주간보고서'){return buildGenericPages(type,doc,[{title:'교육부 주간 활동 요약',body:<SplitText text={doc.summary} path="summary"/>},{title:'부서별 주간 현황',body:<Table heads={['부서','아이들 출석','이번 주 활동','다음 주 활동','특이사항']} rows={EDU_DEPTS.map(dep=>[dep,<AttendanceNumber path={`${dep}_attendance`} value={doc[`${dep}_attendance`]}/>,ev(`${dep}_this`,doc[`${dep}_this`]),ev(`${dep}_next`,doc[`${dep}_next`]),ev(`${dep}_special`,doc[`${dep}_special`])])}/>},{title:'공동 기도제목',body:<PList text={doc.commonPrayer} path="commonPrayer"/>},{title:'공유/지원 요청',body:<PList text={doc.support} path="support"/>}]);}
 if(type==='교육부서 주간보고서') return buildGenericPages(type,doc,[{title:'예배/모임 현황',body:<InfoGrid items={[["부서",ev("subDepartment",doc.subDepartment)],["아이들 출석",<AttendanceNumber path="attendance" value={doc.attendance}/>],["기간",ev("period",doc.period)],["작성자",ev("writer",doc.writer)]]}/>},{title:'이번 주 활동',body:<SplitText text={doc.thisWeek} path="thisWeek"/>},{title:'다음 주 계획',body:<SplitText text={doc.nextWeek} path="nextWeek"/>},{title:'특이사항 및 지원 요청',body:<SplitText text={doc.special} path="special"/>},{title:'기도제목',body:<PList text={doc.prayer} path="prayer"/>}]);
...
function SimpleTimetable({doc}){
  ...
  const rows=rawRows.map((r,i)=>{
    ...
    if(e<=s)e=s+slot;
    return {...r,_index:i,_start:Math.max(startMin,Math.min(endMin,s)),_end:Math.max(startMin+slot,Math.min(endMin,e))};
  ...
  const covered={};
  const scheduleScale=clampFont(doc.scheduleFontScale||100,75,140)/100;
  const tableStyle={'--scheduleTextScale':scheduleScale};
  return <table className="simple-timetable" style={tableStyle}><thead><tr><th>시간</th>{days.map(day=><th key={day}>{day}</th>)}</tr></thead><tbody>{intervals.map((interval,rowIdx)=><tr key={interval.start}><th>{displayTimeLabel(interval.start)}</th>{days.map(day=>{
...
function EventRetreatPlanPreview({doc}){
  ...
  const scheduleDoc={...planDoc,slotMinutes:planDoc.slotMinutes||'60',startHour:planDoc.startHour||'8',endHour:planDoc.endHour||'23',scheduleFontScale:planDoc.scheduleFontScale||'105',timetableHeight:720,maxRowHeight:44};
  ...
  return <>
    ...
    <Page doc={planDoc} className="event-retreat-page event-retreat-schedule-page" pageNo={2}>
      <Header title={titleOf('행사 및 수련회 기획안',planDoc)} meta={meta} doc={planDoc} titlePath="title" metaPath="period"/>
      <Section idx={4} title="일정표" doc={planDoc} block="p2"><ScheduleGrid doc={scheduleDoc}/></Section>
      <Footer/>
    </Page>
    <Page doc={planDoc} className="event-retreat-page event-retreat-budget-page" pageNo={3}>
      <Header title={titleOf('행사 및 수련회 기획안',planDoc)} meta={meta} doc={planDoc} titlePath="title" metaPath="period"/>
...
function AssistantStartPanel({type,setType,setSelected,recentDocs=[]}){
  const shortcuts=[
    ['기본 공지','기본 공지 안내문'],['월간 안내','각부 월간행사 안내'],['주간보고','교육부서 주간보고서']
  ];
...
function RibbonStyleTools({doc,setDoc,type,compact=false}){
  ...
  const recommended={
    ...
    '교육부 주간보고서':['행정 보고형','교육 자료형'],
    '교육부서 주간보고서':['행정 보고형','교육 자료형'],
