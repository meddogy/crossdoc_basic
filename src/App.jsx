...
// BASIC 1.12 최종 편의성 보강판: 일정표 수동 정렬·부서명 빈값 편집·예산 항목 접기 정리
...
function AuthGate({children}){
  ...
  if(status==='signedOut'||status==='sending'||status==='emailSent')return <div className="auth-screen"><form className="auth-card" onSubmit={sendLogin}><div className="auth-logo">✚</div><h1>교회문서키트 BASIC 1.12 작성기</h1><p>구매 시 등록한 이메일로 처음 한 번 로그인해 주세요. 메일의 Sign in 링크는 임시 로그인용이며, 실제로 저장할 주소는 작성기 기본 주소입니다.</p><label className="auth-field"><span>구매자 이메일</span><input type="email" value={formEmail} onChange={e=>setFormEmail(e.target.value)} placeholder="name@example.com" autoComplete="email" disabled={status==='sending'}/></label><button className="auth-primary" disabled={status==='sending'}>{status==='sending'?'로그인 링크 발송 중…':'로그인 링크 받기'}</button>{message&&<div className="auth-message">{message}</div>}{error&&<div className="auth-error">{error}{errorDetail&&<><br/><br/><b>상세 오류</b><br/>{errorDetail}</>}</div>}<details className="auth-debug"><summary>관리자용 설정 확인</summary><p>인증 방식: <code>{AUTH_DEBUG_INFO.mode}</code></p><p>설명: <code>{AUTH_DEBUG_INFO.note}</code></p><p>Redirect URL: <code>{authRedirectUrl()}</code></p></details><small>로그인 후에는 개인 PC에서 로그아웃하지 않고 창만 닫아도 됩니다. 다음부터는 작성기 기본 주소를 즐겨찾기/바로가기/홈 화면에 저장해 사용하세요.</small></form></div>;
...
function WeeklyUnitsBlock({doc,setDoc}){
  ...
  return <section className="v23-tab-block weekly-unit-manager weekly-id-manager"><div className="v23-block-title"><b>부서별 주간 현황</b><span>부서명을 지우고 새로 입력해도 자동으로 “부서1”이 들어가지 않습니다. 입력 후 다른 칸을 누르면 반영됩니다.</span></div><div className="weekly-unit-list">{units.map((unit,i)=><div className="dept-edit weekly-unit-edit" key={unit.id}><div className="weekly-unit-head"><WeeklyUnitNameField value={unit.name} onCommit={v=>patchUnit(unit.id,'name',v)}/>{units.length>1&&<button type="button" onClick={()=>removeUnit(unit.id)}>삭제</button>}</div><div className="grid4 weekly-unit-grid"><Field label="출석/참여(명)" type="number" value={childAttendanceValue(unit.attendance)} onChange={v=>patchUnit(unit.id,'attendance',v)}/><Area label="이번 주 활동" value={unit.thisWeek} onChange={v=>patchUnit(unit.id,'thisWeek',v)}/><Area label="다음 주 계획" value={unit.nextWeek} onChange={v=>patchUnit(unit.id,'nextWeek',v)}/><Area label="특이사항" value={unit.special} onChange={v=>patchUnit(unit.id,'special',v)}/></div></div>)}</div><button type="button" className="btn secondary" onClick={addUnit}>+ 부서/팀 추가</button></section>
}
...
function BudgetRowsTable({rows,onChange,addLabel='항목 추가'}){
  ...
  return <div className="budget-card-editor budget-direct-editor budget-collapsible-editor">
    <div className="budget-card-guide"><span>금액을 직접 입력하는 방식입니다.</span><em>항목이 많아지면 카드 제목만 보고 필요한 항목만 펼쳐 수정할 수 있습니다.</em></div>
