...
function isPlainAttendanceNumber(v){return /^\s*[0-9,]+\s*$/.test(String(v??''))}
function AttendanceNumber({path,value}){
  const raw=String(value??'').trim();
  if(!raw) return <div className="attendance-number-cell" {...fontTargetAttrs(`attendance:${path}`,'출석/참여')}><Edit path={path} value="" className="attendance-number-edit"/></div>;
  if(isPlainAttendanceNumber(raw)){
    const n=raw.replace(/,/g,'');
    return <div className="attendance-number-cell" {...fontTargetAttrs(`attendance:${path}`,'출석/참여 숫자')}><Edit path={path} value={n} className="attendance-number-edit"/><span className="attendance-unit">명</span></div>;
  }
  return <div className="attendance-text-cell" {...fontTargetAttrs(`attendance:${path}`,'출석/참여 내용')}><Edit path={path} value={raw} className="attendance-text-edit"/></div>;
}
...
function summarizeSmartPasteDoc(type,doc){
  ...
  if(type==='부서별 주간보고서')return [`제목: ${doc.reportTitle||'-'}`,`부서: ${doc.subDepartment||'-'}`,`출석/참여: ${doc.attendance||'-'}`,`기간: ${doc.period||'-'}`,`작성자: ${doc.writer||'-'}`,`이번 주 활동: ${doc.thisWeek||'-'}`,`다음 주 계획: ${doc.nextWeek||'-'}`,`특이사항: ${doc.special||'-'}`,`기도제목: ${doc.prayer||'-'}`];
  if(type==='부서 통합 주간보고서')return [`제목: ${doc.reportTitle||'-'}`,`기간: ${doc.period||'-'}`,`작성자: ${doc.writer||'-'}`,`영아부 출석: ${doc['영아부_attendance']||'-'}`,`공동 기도제목: ${splitNonEmpty(doc.commonPrayer).length}줄`];
...
function v23CoreFields(type,doc={}){
  ...
  if(type==='부서별 주간보고서')return [v23Field('제목','reportTitle'),v23Field('부서','subDepartment','select',REPORT_DEPT_OPTIONS),v23Field('출석/참여','attendance'),v23Field('기간','period'),v23Field('작성자','writer')];
...
function mobileCaptureFields(type,doc){
  ...
  if(type==='기본 공지 안내문'){
    base.push(
      {path:'requests',label:'협조 요청',placeholder:'예: 참석 여부는 문자로 회신 부탁드립니다.',kind:'area'},
      {path:'contact',label:'문의처',placeholder:'예: 교육부 담당 교역자 010-0000-0000'}
    );
  }
  return base;
}
function mobileCaptureIsComplete(type){return type==='기본 공지 안내문'}
...
function MobileCapturePanel({type,doc,setDoc,setStage,setMobileSimple,onSave}){
  ...
  const isComplete=mobileCaptureIsComplete(type);
  ...
  return <section className="mobile-capture-panel" aria-label="모바일 작성">
    <div className="mobile-capture-head">
      <div>{isComplete?<em>모바일에서 완성 가능</em>:<em>모바일은 초안만</em>}<b>{isComplete?'공지문은 휴대폰에서 끝까지 작성해 저장하세요':'휴대폰에서는 핵심만 적고 저장하세요'}</b><span>{isComplete?'제목부터 문의처까지 입력한 뒤 바로 PDF·PNG로 저장할 수 있습니다.':'복잡한 표·일정·예산은 PC에서 이어서 정리하는 흐름이 가장 편합니다.'}</span></div>
      <button type="button" onClick={goSave}>저장으로</button>
    </div>
    <div className="mobile-capture-message">{isComplete?<>제목, 일시, 장소, 대상, 안내 내용, 협조 요청, 문의처까지 이 화면에서 모두 입력해 <b>완성한 뒤 저장</b>할 수 있습니다. 표나 일정이 필요한 다른 문서는 PC에서 이어서 작성하세요.</>:<>휴대폰 작성은 <b>완성본 제작</b>이 아니라 <b>현장 메모 저장</b>용으로 잡았습니다. 제목·날짜·장소·핵심 메모만 적어도 PC에서 이어서 문서화할 수 있습니다.</>}</div>
    <div className="mobile-capture-fields">
      {fields.map(f=><label key={f.path}><span>{f.label}</span>{f.kind==='area'?<textarea rows={3} value={String((pathTokens(f.path).reduce((acc,k)=>acc?.[k],doc))??'')} onChange={e=>patch(f.path,e.target.value)} placeholder={f.placeholder}/>:<input value={String((pathTokens(f.path).reduce((acc,k)=>acc?.[k],doc))??'')} onChange={e=>patch(f.path,e.target.value)} placeholder={f.placeholder}/>}</label>)}
    ...
    <div className="mobile-capture-actions"><button type="button" onClick={()=>{onSave?.();goSave();}}>{isComplete?'저장하고 저장화면으로':'임시 저장 후 저장화면'}</button><button type="button" onClick={goPreview}>미리보기 확인</button><button type="button" className="subtle" onClick={openDetail}>세부 서식은 필요할 때만</button></div>
  </section>
}
...
const GUIDE_DOC_TIPS={
  ...
  '부서별 주간보고서':['부서명·기간·작성자를 먼저 입력한 뒤 이번 주 활동과 다음 주 계획을 정리하세요.','출석/참여 칸에는 숫자뿐 아니라 “학생 24명 / 교사 6명”, “온라인 3명 포함”처럼 글자와 숫자를 함께 입력할 수 있습니다.','표 안 글씨가 길면 문서 기본도구에서 글씨 크기를 작게 조정하세요.'],
