...
function defaultStyleFor(type){
  const base={theme:'클래식 네이비',preset:'행정 보고형',...THEMES['클래식 네이비'],fontScale:100};
  if(type==='각부 월간행사 안내')return {...base,...presetStylePatch('월간 일정형'),fontScale:100};
  return base;
}
...
function withBase(type,d){
  ...
  // v2.24: 월간행사 안내는 기본 색상이 회색 계열로 바뀌지 않도록
  // 기존 저장값이 기본 행정 보고형이면 월간 일정형 네이비로 복구합니다.
  if(type==='각부 월간행사 안내' && (!d?.style || normalizePreset(d?.style?.preset)==='행정 보고형')){
    Object.assign(style,presetStylePatch('월간 일정형'));
  }
...
function RibbonStyleTools({doc,setDoc,type,compact=false}){
  ...
  const recommended={
    '각부 월간행사 안내':['월간 일정형','카톡 공지형','행사 포스터형'],
