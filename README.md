...
1.18 관리자 승인 기능을 사용하려면 아래 서버 전용 환경변수를 추가해야 합니다.
...
## 1.18 수정 내용
- 미리보기에서 드래그로 선택된 글씨 영역이 다른 곳을 클릭하거나 ESC를 누르면 해제되도록 수정했습니다.
- 미리보기 하단의 오해될 수 있는 안내 문구를 제거했습니다.
- 글자 메뉴의 선택 안내 문구를 실제 기능에 맞게 단순화했습니다.
- 베타 신청서에 연락처 필드를 추가했습니다. 기존 Supabase 테이블에는 `alter table ... add column if not exists phone text;`를 실행하면 됩니다.


## BASIC 1.19 수정
- 미리보기 직접 수정 시 Enter 한 번으로 바로 다음 줄로 넘어가도록 보강했습니다.
- 미리보기 하단의 글자 선택 안내 문구를 실제 기능 기준으로 정리했습니다.
